...
class Order(Base):
    ...
    employee = relationship(
        "User", foreign_keys="[Order.employee_id]", back_populates="employee_orders"
    )
    administrator = relationship(
        "User", foreign_keys="[Order.administrator_id]", back_populates="admin_orders"
    )
    services = relationship(
        "Service", secondary="order_service", back_populates="orders"
    )
